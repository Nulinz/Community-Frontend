...
const FormInput = ({ field, value, onChange, inputName }) => {
  ...
  if (field.type === "file") {
    const isEditingMode = typeof value === 'string' && value.length > 0;
    const isNewFileSelected = value instanceof File;
    const BASE_URL = import.meta.env.VITE_API_URL || "http://localhost:5000";

    return (
      <div className="flex flex-col gap-3">
        {(isEditingMode || isNewFileSelected) && (
          <div className="w-32 h-32 rounded bg-gray-100 border border-gray-200 overflow-hidden shrink-0">
            <img 
              src={isNewFileSelected ? URL.createObjectURL(value) : `${BASE_URL}/${value}`} 
              alt="Preview" 
              className="w-full h-full object-cover"
            />
          </div>
        )}
        <div className="flex border border-gray-200 rounded overflow-hidden h-10 w-full max-w-sm font-source transition focus-within:border-blue-400">
          ...
          <span className="flex-1 min-w-0 bg-[#fcfcfc] px-3 md:px-4 py-2.5 text-gray-400 text-xs truncate">
            {isNewFileSelected ? value.name : (isEditingMode ? "Keep existing profile image" : "No file chosen")}
          </span>
        </div>
      </div>
    );
...
const SeminarForm = () => {
  ...
  const sectionMap = seminarFormConfig.reduce((acc, section) => {
  ...
  const buildPayload = () => {
    ...
    seminarFormConfig.forEach((section) => {
  ...
  const validateForm = () => {
    for (const section of seminarFormConfig) {
      if (section.type === "static") {
        for (const field of section.fields) {
          if (field.required !== false) {
            const value = staticData[field.name];
            const isEditMode = !!editData;
            // If edit mode and field is file and we have string (existing image), it's valid
            if (isEditMode && field.type === "file" && typeof value === 'string' && value.trim() !== '') {
                continue;
            }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(editData ? "Seminar updated successfully" : "Seminar saved successfully");
      if (editData) {
        navigate(`/admin/seminar-profile/${editData._id || editData.id}`, { replace: true });
      } else {
  ...
  return (
    <div className="mx-auto bg-gray-50">
      ...
      >
        {seminarFormConfig.map((section, sectionIndex) => (
