...
const FormInput = ({ field, value, onChange, inputName }) => {
  ...
  if (field.type === "file") {
    const isImage = value && (value instanceof File || typeof value === "string");
    const previewUrl = value instanceof File ? URL.createObjectURL(value) : (typeof value === "string" ? `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/${value}` : null);

    return (
      <div className="space-y-2">
        <div className="flex border border-gray-200 rounded overflow-hidden h-10 w-full font-source transition focus-within:border-blue-400 bg-white">
          ...
          <span className="flex-1 min-w-0 bg-[#fcfcfc] px-3 md:px-4 py-2.5 text-gray-400 text-xs truncate">
            {value instanceof File ? value.name : (typeof value === "string" ? "Current Image" : "No File Chosen")}
          </span>
        </div>
        {previewUrl && isImage && (
...
const ConferenceForm = () => {
  ...
  const sectionMap = conferenceFormConfig.reduce((acc, section) => {
  ...
  const buildPayload = () => {
    ...
    conferenceFormConfig.forEach((section) => {
  ...
  const validateForm = () => {
    for (const section of conferenceFormConfig) {
      if (section.type === "static") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Failed to save conference");
    } finally {
  ...
  return (
    <div className="mx-auto bg-gray-50">
      ...
      >
        {conferenceFormConfig.map((section, sectionIndex) => (
