...
const eventFormConfig = [
  ...
  
  {
    ...
    fields: [
      ...
      { name: "allowedDepartments", label: "Allowed Departments", type: "select", options: ["CS", "IT", "ECE", "EEE"] },
      ...
      <div className="space-y-2">
        <div className="flex border border-gray-200 rounded overflow-hidden h-10 w-full font-source transition focus-within:border-blue-400">
          ...
          <span className="flex-1 min-w-0 bg-[#fcfcfc] px-3 md:px-4 py-2.5 text-gray-400 text-xs truncate">
            {value instanceof File ? value.name : isExistingImage ? "Current image (change to replace)" : "No File Chosen"}
          </span>
        </div>
        {isExistingImage && (
          <div className="flex items-center gap-3 mt-1">
            ...
            />
            <span className="text-xs text-gray-400">Current cover image</span>
          </div>
        )}
        {value instanceof File && (
          <div className="flex items-center gap-3 mt-1">
            <img
              ...
              className="h-16 w-24 object-cover rounded border border-gray-200"
            />
            <span className="text-xs text-gray-400">New image preview</span>
          </div>
        )}
      </div>
    );
...
const EventForm = () => {
    ...
    if (editData) {
      ...
      eventFormConfig
        ...
        .forEach((section) => {
          section.fields.forEach((field) => {
            if (field.type === "file") {
              // Seed file field with existing image path string so preview is shown
              data[field.name] = editData[field.name] ?? "";
            } else {
              // Use ?? so empty strings from DB are preserved (not replaced by default)
              const val = editData[field.name];
              data[field.name] = (val !== undefined && val !== null) ? val : getInitialFieldValue(field);
            }
          });
        });
      // Preserve ID for update
      if (editData._id || editData.id) {
        data.id = editData._id || editData.id;
      }
      // Explicitly carry over description in case it was missed
      if (editData.description !== undefined) {
        data.description = editData.description;
      }
      return data;
    }
    return createInitialStaticData();
  });
  ...
  const sectionMap = eventFormConfig.reduce((acc, section) => {
  ...
  const buildPayload = () => {
    ...
    // Include the id first for update operations
    if (staticData.id) {
      payload.id = staticData.id;
    }

    eventFormConfig.forEach((section) => {
      if (section.type === "static") {
        section.fields.forEach((field) => {
          const payloadKey = field.payloadKey || field.name;
          const rawValue = staticData[field.name];
          // For file fields: only include if it's an actual File object (not an existing path string)
          if (field.type === "file") {
            if (rawValue instanceof File) {
              payload[payloadKey] = rawValue;
            }
            // skip string paths — backend retains old image if no new file uploaded
          } else {
  ...
  const validateForm = () => {
    ...
    for (const section of eventFormConfig) {
      if (section.type === "static") {
        for (const field of section.fields) {
          if (field.required !== false) {
            const value = staticData[field.name];
            // In edit mode, file fields are optional (existing image is kept)
            if (field.type === "file" && isEdit) continue;
      ...
      if (section.type === "dynamic") {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          for (const field of section.fields) {
            ...
            // Additional validations for phone and email
            if (hasMeaningfulValue(value)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(editData ? "Event updated successfully" : "Event saved successfully");
      if (editData) {
        navigate(-1);
      } else {
        setStaticData(createInitialStaticData());
    ...
    <div className="mx-auto bg-gray-50">
      ...
      >
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-xl font-bold text-gray-800">{editData ? "Edit Event" : "Add Event"}</h2>
        </div>
        {eventFormConfig.map((section, sectionIndex) => (
