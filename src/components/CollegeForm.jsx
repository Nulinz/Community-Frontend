...
import { createCollege } from "../services/admin/adminServices";
...
const CollegeForm = () => {
  ...
  useEffect(() => {
    if (editData) {
      setCollegeId(editData._id || editData.id);
      ...
      Object.keys(newStaticData).forEach((key) => {
        ...
        // Handle field mapping aliases from User profile/flattened data
        if (key === "mailId") val = editData.email;
        if (key === "phoneNumber") val = editData.phone;

        if (val !== undefined && val !== null) {
          if (key === "establishedYear" && val) {
  ...
  const sectionMap = collegeFormConfig.reduce((acc, section) => {
  ...
  const buildPayload = () => {
    ...
    collegeFormConfig.forEach((section) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (collegeId) payload.id = collegeId; // Include ID for update

      const formData = buildFormData(payload);
      await createCollege(formData);
      toast.success(collegeId ? "College updated successfully" : "College created successfully");
  ...
  return (
    <div className="mx-auto bg-gray-50">
      ...
      >
        {collegeFormConfig.map((section, sectionIndex) => (
        ...
        <div className="flex flex-col-reverse md:flex-row justify-end gap-3 md:gap-4 pt-4 md:pt-6">
          ...
          >
            {isSubmitting ? "Saving..." : collegeId ? "Update" : "Save"}
          </button>
        </div>
      </form>
    </div>
