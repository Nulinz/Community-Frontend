...
const FreelanceForm = () => {
  ...
  const sectionMap = freelanceFormConfig.reduce((acc, section) => {
  ...
  const buildPayload = () => {
    ...
    freelanceFormConfig.forEach((section) => {
  ...
  const validateForm = () => {
    for (const section of freelanceFormConfig) {
      if (section.type === "static") {
      ...
      if (section.type === "dynamic") {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          for (const field of section.fields) {
            ...
            // Only first 3 rows are required for dynamic sections
            if (i < 3) {
              if (!hasMeaningfulValue(value)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Failed to save freelance");
    } finally {
  ...
  return (
    <div className="mx-auto bg-gray-50">
      ...
      >
        {freelanceFormConfig.map((section, sectionIndex) => (
